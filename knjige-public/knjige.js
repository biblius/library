...
const appendBorrowers = () => {
  for (let i = 0; i < borrowers.length; i++) {
    ...
    borrowerEntry.classList.add("card");
    ...
    // Mozes datum pretvoriti u date objekt i onda ga prikazati u lokalnom formatu.
    // Ovdje sam izricito rekla 'hrv' jer mi je browser i sve na engleskom pa da se vidi razlika.
    pDate.innerHTML = `Date: ${new Date(
      borrowers[i].dateBorrowed
    ).toLocaleDateString("hrv")}`;
    ...
    // Obrati pozornost na tooltip i kako se renderira. Trenutno taj tooltip postoji u DOM-u
    // i nekada smeta kod dodavanja biljeski.
    // Alternativa je da koristis recimo title atribut, on je nesto kao default browser tooltip.
